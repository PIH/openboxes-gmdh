...
SELECT product_code, SUM(temp.quantity) FROM 
(
    SELECT 
        product_code,
    ...
    ) AS is2 ON inventory_snapshot.location_id = is2.location_id AND inventory_snapshot.product_id = is2.product_id AND inventory_snapshot.date = maxdate
    WHERE location.name = 'Miami Warehouse'
    GROUP BY inventory_snapshot.product_id, inventory_snapshot.location_id
) as temp
GROUP BY temp.product_code;
