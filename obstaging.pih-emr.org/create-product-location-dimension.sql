...
JOIN location_classification ON location.id = location_classification.location_id
WHERE classification = 'Level 2';

ALTER TABLE `dim_product_location` ADD INDEX `product_location_idx` (`product_id`, `location_id`);
#ALTER TABLE `inventory_snapshot` ADD INDEX `inventory_snapshot_idx` (`product_id`, `location_id`, `date`);
